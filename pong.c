...
void main_render(int ball_x, int ball_y, int rocket_p1, int rocket_p2,
...
int move_r1(char ch, int r1_y, int vector_ball_x) {
  if (vector_ball_x == -1) {
    if ((ch == 'z' || ch == 'Z') && r1_y < ROW - 3)
      r1_y++;
    else if ((ch == 'a' || ch == 'A') && r1_y > 2)
      r1_y--;
  }
  return r1_y;
}
...
int move_r2(char ch, int r2_y, int vector_ball_x) {
  if (vector_ball_x == 1) {
    if ((ch == 'm' || ch == 'M') && r2_y < ROW - 3)
      r2_y++;
    else if ((ch == 'k' || ch == 'K') && r2_y > 2)
      r2_y--;
  }
  return r2_y;
}
...
int hit_p1(int x_ball, int y_ball, int vector, int r_coord) {
  if (x_ball == 2 &&
      (y_ball == r_coord || y_ball == r_coord - 1 || y_ball == r_coord + 1))
    vector *= -1;
  return vector;
}
...
char is_game_over(char input, int score_p1, int score_p2) {
  ...
  if (input == 'q' || input == 'Q')
    result = 0;
  else if (score_p1 == GAME_OVER_SCORE || score_p2 == GAME_OVER_SCORE)
    result = 0;
  return result;
}
