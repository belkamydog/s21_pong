...
void field_render(int y, int x, char is_printed){
    if (is_printed == 0){
        if (y == 0 || y == (ROW-1)) printf("%c", HORISONTAL_BORDER);
        else if (x == 0 || x == (COL-1)) printf("%c", VERTICAL_BORDER);
        else printf(" ");
    }
}
...
void show_final_score(int score_p1, int score_p2){
    if (score_p1 > score_p2) printf("Player 1 win\n");
    else if (score_p1 == score_p2) printf("Draw\n");
