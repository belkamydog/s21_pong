...
void init_game(Game *game){
    ...
    game->game_over = 1;
    game->user_input = ' ';
    game->ball_speed = 500000;
}

void is_game_over(char input, Game *game){
    char result = 1;
    if (input == 'q' || input == 'Q') result = 0;
    else if (game->player_1.score == GAME_OVER_SCORE || game->player_2.score == GAME_OVER_SCORE) result = 0;
...
void moove_ball(Game *game){
    ...
    if (game->ball.x == 2 && (game->ball.y == game->player_1.position || game->ball.y == game->player_1.position-1 || game->ball.y == game->player_1.position+1 )) game->ball.vector_x *= -1;
    if (game->ball.x == (COL-3) && (game->ball.y == game->player_2.position || game->ball.y == game->player_2.position-1 || game->ball.y == game->player_2.position+1 )) game->ball.vector_x *= -1;
...
void main_render(Game game){
    for (int y = 0; y < ROW; y++){
        for (int x = 0; x < COL; x++){
            ...
            printed = rockets_render(x, y, game.player_1.position, game.player_2.position, printed);
...
void moove_rockets(Game *game){
    ...
    if (game->ball.vector_x == -1){
        if ((ch == 'z' || ch == 'Z') && game->player_1.position < ROW-3) game->player_1.position += 1;
        else if ((ch == 'a' || ch == 'A') && game->player_1.position > 2) game->player_1.position -= 1;
    }
    else if (game->ball.vector_x == 1){
        if ((ch == 'm' || ch == 'M') && game->player_2.position < ROW-3) game->player_2.position += 1;
        else if ((ch == 'k' || ch == 'K') && game->player_2.position > 2) game->player_2.position -= 1;
    }
}
